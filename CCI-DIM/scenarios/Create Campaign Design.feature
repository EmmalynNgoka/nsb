...
Scenario: Create Campaign Design 
	...
	When I select "Set As Confirmed" from Campaign Status drop down
	Then verify campaing status should be "Confirmed" 
	When I click on Pause Campaign button 
	Then verify Pause Campaign dialog should be displayed
	When I select confirm button 
	Then verify campaing status should be "Offline - Paused" 
	When I select "Distribution" tab on create campaign page 
	Then verify distribution message for zero allocation
	When I select "Set As Confirmed" from Campaign Status drop down
	Then verify campaing status should be "Confirmed" 
# 	And verify distribution message contains the goal "1080"
# 	When I click on Pause Campaign button 
#	Then verify Pause Campaign dialog should be displayed
#	When I select confirm button 
#	Then verify campaing status should be "Offline - Paused"
# 	When I navigate to the "campaign" tab 
# 	And delete the current campaign
